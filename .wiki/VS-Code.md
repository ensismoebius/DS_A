...
Ambos os projetos — `javafxLinux_A` e `javafxWindows_A` — estão configurados
...
Abra **a pasta do projeto** (`javafxLinux_A` ou `javafxWindows_A`) diretamente
...
- **`javafxLinux_A`** usa comandos Unix (`mkdir -p`, `find`, `xargs`).
- **`javafxWindows_A`** traz um override `windows` em PowerShell para que o build
...
- `javafxLinux_A` empacota libs nativas Linux (`.so`); `javafxWindows_A`
